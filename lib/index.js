...
GetReviewers.prototype.getComments = function getComments(number, cb) {
  var self = this
...
GetReviewers.prototype.getReviewers = function getReviewers(number, cb) {
  var self = this
  self.github.pullRequests.getReviews({
    owner: 'nodejs'
  , repo: this.opts.nodePrivate ? 'node-private' : 'node'
  , number: number
  }, function (err, reviewers) {
    reviewers = reviewers.filter(function(review) {
      if (review.state !== 'APPROVED') {
        err = 'PR Not Approved!'
      }
      return review.state === 'APPROVED'
        && users.hasOwnProperty(review.user.login)
    }).reduce(function (set, review) {
      if (!set.hasOwnProperty(review.user.login)) {
        set[review.user.login] = {
...
GetReviewers.prototype.fetchPR = function fetchPR(number, cb) {
  ...
  async.parallel([
    self.getComments.bind(self, number)
  , self.getReviewers.bind(self, number)
